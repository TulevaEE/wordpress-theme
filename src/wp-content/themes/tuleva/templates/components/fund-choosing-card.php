<div class="card-body text-center pt-5">
    ...
    <div class="bg-white mb-4">
        <?php if ( have_rows('link') ) while ( have_rows('link') ) : the_row(); ?>
            <?php $link = get_sub_field('link'); ?>
            <?php if ($link) { ?>
                <a href="<?php echo $link ?>" class="text-uppercase text-medium d-block mb-4"><?php the_sub_field('text'); ?></a>
            <?php } ?>
        <?php endwhile; ?>
            <?php if ( have_rows('button') ) while ( have_rows('button') ) : the_row(); ?>
            <?php $link = get_sub_field('link'); ?>
            <?php if ($link) { ?>
                <a href="<?php echo $link ?>" class="btn btn-primary btn-lg btn-block" id="fund-choose-stocks"><?php the_sub_field('text'); ?></a>
            <?php } ?>
        <?php endwhile; ?>
    </div>
    <hr class="mb-4">
